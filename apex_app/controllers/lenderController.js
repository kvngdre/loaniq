...
const lender = {
  ...
  getAll: async function () {
    return await Lender.find().select('-password -otp').sort('companyName');
  },
  ...
  changePassword: async function (requestBody) {
    try {
      ...
      if (isSimilar)
        throw new Error('Password is too similar to old password.');
  ...
  update: async function (id, requestBody) {
    try {
      const lender = await Lender.findOneAndUpdate({ _id: id }, requestBody, {
        new: true
      });
      if (!lender) throw new Error('Lender not found.');
