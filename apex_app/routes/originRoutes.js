...
const originViewController = require('../controllers/originController');
...
router.post('/', verifyToken, verifyRole('origin-master'), async (req, res) => {
    ...
    const newCustomer = await originViewController.create(req.body);
...
router.get('/', verifyToken, verifyRole('origin-master'), async (req, res) => {
    const customers = await originViewController.getAll(req.body);
...
router.get('/one', verifyToken, verifyRole('origin-master'), async (req, res) => {
    const customer = await originViewController.getOne(req.body);
...
router.patch('/:id', verifyToken, verifyRole('origin-master'), async (req, res) => {
    ...
    const updatedCustomer = await originViewController.edit(req.params.id, req.body);
...
router.delete('/:id', verifyToken, verifyRole('origin-master'), async (req, res) => {
    const deletedCustomer = await originViewController.delete(req.params.id);
