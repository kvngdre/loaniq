...
router.get('/', verifyToken, verifyRole(['Admin', 'Lender']), async (req, res) => {
    const transactions = await transactionController.getAll(req.user, req.body);
    if(transactions instanceof Error) return res.status(404).send(transactions.message);

    return res.status(200).send(transactions);
});

router.get('/:id', verifyToken, verifyRole(['Admin', 'Lender']), async (req, res) => {
